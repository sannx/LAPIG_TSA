...
#Apply filter Max min in matrix
  ST = Sys.time()
  ndvi_mm = t(apply(ndvi[,361:383], 1, MaxMinFilter, n = 2))
  Sys.time() - ST
...
for (i in 0:15) {
  ...
  ndvi_df = as.data.frame(ndvi_mm)
  gmm_1 = mvnormalmixEM(ndvi_mm)

  
  plot(gmm_1, density=TRUE, cex.axis=1.4, cex.lab=1.4, cex.main=1.8,
...
####################################################################################
  ...
  r = brick("F:\\DATASAN\\raster\\BHRV\\SZMT_BHRV_ALL.tif")
  names(r)
